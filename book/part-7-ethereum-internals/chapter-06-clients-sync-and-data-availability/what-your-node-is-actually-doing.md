...
**Client** 是参与链同步、验证和对外提供接口的软件实现。直觉上它像你运行在本地或服务商后台的链上操作系统。工程上这意味着“节点怎么看链”其实取决于具体 client 的实现与同步状态。

**同步模式（Sync Mode）** 是节点选择怎样追上链状态的同步策略。直觉上它像赶进度时选择是完整重看全部历史，还是先抓重点再逐步补。工程上这意味着不同 sync mode 会影响你拿到数据的速度、完整度和磁盘开销。

**Execution Client** 是负责交易执行、状态计算和 EVM 逻辑的客户端。直觉上它像真正负责算账和跑合约的引擎室。工程上这意味着很多你以为是“链本身”的执行行为，实际上是 execution client 在忠实地实现协议规则。
