...
**Gas Limit** 是单笔交易或单个区块允许消耗的 gas 上限。直觉上它像系统为资源使用设下的天花板。工程上这意味着交易能不能被纳入、函数能不能随规模继续执行，都直接受 gas limit 约束。

**Base Fee** 是区块对 gas 统一收取的基础价格。直觉上它像当前区块的统一门票底价。工程上这意味着用户想进区块，首先要满足 base fee，而不是只看自己愿意付多少小费。

**Priority Fee** 是用户额外支付给打包方的排序激励。直觉上它像为了更快上场而额外给出的加急费。工程上这意味着同样逻辑的交易，在拥堵环境里能否优先被纳入，很大程度受 priority fee 影响。
