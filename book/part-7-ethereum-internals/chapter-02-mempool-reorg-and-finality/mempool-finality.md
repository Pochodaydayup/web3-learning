...
**Mempool** 是待打包交易在网络里传播和排队的区域。直觉上它像交易正式写进账本前的公开候场区。工程上这意味着很多前端看到的 pending、替换和排序竞争，都发生在 mempool 这一层。

**Reorg** 是链头短暂回退并被另一条更优分支替换的现象。直觉上它像你刚写好的账页又被撤回，换成了另一版。工程上这意味着低确认数下看到的结果未必稳定，事件和状态都可能短暂出现后再消失。

**Finality** 是结果稳定到不太可能再被回滚的状态。直觉上它像系统终于愿意把这件事认定为定局。工程上这意味着钱包、交易页和协议判断，对 finality 的要求会直接影响产品风险取舍。
