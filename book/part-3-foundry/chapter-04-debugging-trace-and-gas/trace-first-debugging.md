...
**Trace** Trace 是交易或测试执行过程中逐步展开的调用与操作轨迹。直觉上它像把黑盒执行过程拆成一格一格的监控录像。工程上这意味着当合约报错或状态异常时，trace 往往比只看报错字符串更能说明问题发生在哪一层。
...
**Gas Snapshot** Gas snapshot 是记录关键路径 gas 消耗并用于回归比较的基准数据。直觉上它像性能体检报告，告诉你这次改动有没有悄悄把成本抬高。工程上这意味着优化不是靠感觉，而是要有可重复对比的成本基线。
