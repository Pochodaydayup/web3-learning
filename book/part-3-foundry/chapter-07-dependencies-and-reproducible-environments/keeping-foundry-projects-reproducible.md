...
**依赖锁定（Dependency Pinning）** 依赖锁定是把依赖版本固定在明确可复现范围内的做法。直觉上它像把实验条件钉死，避免今天和明天拿到不同材料。工程上这意味着版本一旦漂移，测试结果、编译输出和部署风险都可能随之变化。

**版本漂移（Version Drift）** 版本漂移指团队成员或环境之间依赖、编译器和工具版本逐渐不一致的现象。直觉上它像大家都觉得在做同一件事，但脚下的地板其实已经不是同一块。工程上这意味着很多“我这里能跑、你那里不行”的问题，本质上不是代码逻辑，而是环境漂移。

**确定性环境（Deterministic Setup）** 确定性环境是让不同机器和不同时间都能得到同样构建结果的环境约束。直觉上它像把实验室条件控制到足够稳定，别人照着操作也会得出同样结果。工程上这意味着可复现不是锦上添花，而是交付、审计和长期维护的基础。
