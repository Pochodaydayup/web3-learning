...
**CI** CI 是在代码合并前自动运行检查、测试和构建的持续集成流程。直觉上它像一位不会疲劳的门卫，确保明显问题不会轻易混进主干。工程上这意味着合约团队的稳定性，很大程度来自自动化验证而不是大家“平时都很小心”。

**发布门禁（Release Gate）** 发布门禁是发版前必须通过的一组检查和准入条件。直觉上它像真正上线前的最后一道闸门，不让“差不多就行”的状态直接进生产。工程上这意味着部署不是脚本能跑就结束，而是要结合测试、权限、配置和风险确认一起判断。

**自动化（Automation）** 自动化是把重复工程动作沉淀为可重复执行流程的实践。直觉上它像把依赖记忆和手速的工作换成稳定机器动作。工程上这意味着自动化做得越好，交付质量就越少依赖个人状态。
