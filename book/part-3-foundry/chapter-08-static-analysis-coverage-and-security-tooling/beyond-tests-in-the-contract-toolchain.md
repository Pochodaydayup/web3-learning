...
**静态分析（Static Analysis）** 静态分析是在不真正运行代码的前提下扫描结构和风险模式的分析方式。直觉上它像还没开机就先检查线路图，看哪些地方天生可疑。工程上这意味着静态分析能提前发现权限、重入和危险调用模式，但不能替代运行时验证。

**覆盖率（Coverage）** 覆盖率是测试到底触达了多少代码路径的度量。直觉上它像告诉你巡检时走过了多少房间。工程上这意味着覆盖率有助于发现明显的测试盲区，但高覆盖率本身不等于高质量测试。

**安全工具链（Security Tooling）** 安全工具链是围绕静态分析、覆盖率和风险扫描建立的辅助安全工具集合。直觉上它像一组不同角度的安检设备，各自捕捉不同类型的风险信号。工程上这意味着成熟团队不会把安全寄托在单一工具或单一测试方式上。
