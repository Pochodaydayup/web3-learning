...
**知识库（Knowledge Base）** 是围绕某个领域持续积累、可检索和可复用的个人知识系统。直觉上它像你给自己建的第二大脑。工程上这意味着学习如果没有沉淀进知识库，很多理解都会在几周后迅速蒸发。

**笔记仓库（Notes Repository）** 是用仓库结构管理学习记录、源码阅读和知识沉淀的载体。直觉上它像把零散笔记整理成一座可以持续扩建的图书馆。工程上这意味着仓库化会让你的学习材料更易检索、连接和公开展示。

**公开日志（Public Log）** 是持续对外展示学习过程、进展和复盘的记录流。直觉上它像你把成长过程实时留在外界可见的黑板上。工程上这意味着公开日志既帮助自我约束，也能逐步积累外部信任。
