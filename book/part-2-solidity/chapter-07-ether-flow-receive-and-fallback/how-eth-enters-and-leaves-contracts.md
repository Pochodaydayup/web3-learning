...
**`receive`** 是在合约收到纯 ETH 且没有 calldata 时触发的兜底入口。直觉上它像“有人直接把钱打进门口时的收款台”。工程上这意味着如果你希望明确处理单纯收款路径，就应该清楚地区分 `receive` 和普通业务函数。

**`fallback`** 是在调用找不到匹配函数，或进入更泛化兜底路径时触发的入口。直觉上它像“没有预约也闯进来的访客会先被带到前台”。工程上这意味着你必须非常明确这个入口是否允许收 ETH、允许执行什么逻辑，以及它会不会扩大攻击面。

**外部调用（External Call）** 是合约把控制权短暂交给外部地址的一次交互。直觉上它像你把门打开，让另一方有机会在你流程尚未结束时做反应。工程上这解释了为什么 ETH 转出路径总要结合会计顺序、重入风险和失败处理一起设计。
