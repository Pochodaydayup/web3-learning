...
**`mapping`** 是 Solidity 最典型的按 key 查 value 的状态结构。直觉上它像一个只在你拿着准确钥匙时才愿意开门的柜子，而不是一张能随手翻全表的数据库表。工程上这意味着 `mapping` 很适合点查，但不会自动维护“所有 key 的列表”。

**枚举（Enumeration）** 指把某类实体完整列出来、遍历出来的能力。直觉上它像你想知道“系统里到底有多少人、多少订单、多少仓位”的需求。工程上这不是默认能力，而是一项需要额外状态设计和 gas 成本支撑的功能。

**辅助索引（Auxiliary Index）** 是为了支持枚举而额外维护的数组、集合或顺序结构。直觉上它像给主状态再做一份目录页，好让你以后能按集合翻找。工程上这意味着每增加一份索引，写路径都会更贵、更复杂，因此你必须先确认这个枚举需求到底值不值得长期承担。
