...
**Proxy** 是对外长期稳定存在、负责保存状态和接收用户调用的入口合约。直觉上它像门面与仓库的结合体，外界始终敲这扇门，但门后执行哪套逻辑可以调整。工程上这意味着真正持久的业务状态通常落在 proxy 的存储里。

**Initializer** 是替代 constructor 在代理上下文里完成初始化的函数。直觉上它像“真正第一次启动系统时要执行的安装脚本”，因为普通 constructor 不会自动在 proxy 状态里生效。工程上这意味着初始化是否只能执行一次，会直接决定升级系统是否安全。

**Storage Layout** 是状态变量最终映射到槽位的排列规则。直觉上它像数据库列顺序加上底层寻址规则，一旦线上数据已经存在，就不能随意重排。工程上这解释了为什么升级时新增、删除和调整变量顺序都是高风险操作。
