...
**Storage** 是合约真正写进链上并会长期保留下来的持久化数据区。直觉上它像合约自己的硬盘，写进去的内容会跨交易继续存在。工程上这意味着任何 storage 写入都会影响 gas 成本、状态布局和升级安全。

**Memory** 是一次函数执行过程中临时使用的数据区。直觉上它像当前调用拿来摆放中间草稿的工作台，调用结束后内容就消失。工程上这意味着 memory 适合处理中间结果和副本，但不适合承载需要长期保存的业务事实。

**Calldata** 是外部调用带进合约的只读输入区，也是前端 ABI 编码参数最终落脚的地方。直觉上它像用户交给这次调用的一份封装包裹，合约可以拆开读，但不会原地改写。工程上这解释了为什么很多外部函数参数会优先使用 `calldata`，以避免不必要的数据复制。
