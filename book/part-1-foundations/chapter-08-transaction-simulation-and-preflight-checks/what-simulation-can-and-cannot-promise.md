...
**Simulation** 是基于当前节点视角和当前状态快照，对一笔调用做的一次预演执行。直觉上它像正式上场前的彩排，能帮你提前看到明显的失败路径。工程上这意味着 simulation 对“此刻能不能执行”很有价值，但它不对几秒后真实上链时的状态变化做担保。

**`estimateGas`** 是节点为某笔交易探索一条可执行路径后给出的 gas 需求探测结果。直觉上它更像“试探性测量”，而不是一纸保证合同。工程上这意味着你通常要把它当基线，再结合 buffer 和失败重试策略，而不是把它当固定真值。

**Preflight Check** 是发送前用来排除明显错误、补充风险提示的一组校验动作。直觉上它像登机前的安检，能筛掉很多已知问题，但不能控制起飞后的天气。工程上这意味着前端最该用 preflight 去解决链不对、余额不足、参数不合法这类确定性错误，而不是向用户暗示“现在已经百分之百会成功”。
