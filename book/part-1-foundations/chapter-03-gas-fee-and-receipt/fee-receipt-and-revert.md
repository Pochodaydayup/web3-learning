...
**Gas Limit** 是你愿意为这次执行预留的最大资源预算，而不是系统提前向你保证的实际花费。直觉上它像“给这次链上动作准备的一张上限卡”，防止执行无限消耗资源。工程上这意味着 gas limit 过低会让交易中途耗尽而失败，设得更高则只是给执行留出空间，不代表一定全部花掉。

**Receipt** 是交易执行完成后留下的结果摘要，里面集中记录了 `status`、`gasUsed`、区块位置和 logs。直觉上它更像一次执行的结案报告，而不是业务解释器。工程上这意味着你应先用 receipt 判断交易到底有没有成功落链、花了多少、发出了哪些日志，再去结合状态和前端上下文解释业务结果。

**Revert** 是执行过程中显式触发的失败语义，会让这笔交易的状态修改回滚。直觉上它像“流程走到一半发现条件不成立，于是撤销结果但保留过程消耗”。工程上这解释了为什么失败交易通常仍然要付费，以及为什么前端需要把“业务失败”和“什么都没发生”明确区分开。
