...
**RPC 节点** 是某个具体客户端对外暴露的链上接口，而不是抽象意义上的唯一真理源。直觉上它更像“某个观察者眼里的当前链头”，不同观察者可能在同一时刻看到略有差别的画面。工程上这意味着你读到的结果天然带着节点同步进度、服务商架构和负载状况的烙印。

**Block Tag** 是告诉节点“请基于哪个区块状态回答我”的读取锚点。直觉上它像给这次查询先钉住一个观察时间点，避免不同请求各自漂移。工程上这意味着关键页面如果想减少抖动，往往要尽量让一组查询基于同一个 block number 或 block tag。

**索引器（Indexer）** 是把链上原始数据消费、整理、缓存后再提供查询能力的链下系统。直觉上它像一层帮你把原始账本重排成产品视图的中间数据库。工程上这意味着索引器通常更适合列表、聚合和统计，但它天然会带来同步延迟和与链上直读的短时分歧。
