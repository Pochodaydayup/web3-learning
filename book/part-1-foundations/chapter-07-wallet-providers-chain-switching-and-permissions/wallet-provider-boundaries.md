...
**钱包（Wallet）** 是用户持有私钥、确认签名和批准交易的能力容器。直觉上它更像“代表用户做最终授权决定的门卫”，而不是一个普通 UI 组件。工程上这意味着钱包关心的是身份与同意，而不是替你的业务状态管理兜底。

**Provider** 是 dApp 与钱包或节点进行通信的桥接接口。直觉上它像一条可变的外部管道，你通过它请求账户、链 ID、签名和 RPC 能力。工程上这意味着 provider 需要被当成外部依赖处理，账户变化、链变化和断开连接都可能随时发生。

**权限动作（Permission Action）** 指连接账户、切换链、签消息和发交易这几类不同语义的授权请求。直觉上它们像四种完全不同的门，不应该被一句“请授权”混成一类。工程上这意味着前端必须清楚当前到底缺的是哪一步权限，否则弹窗再多，用户也只会越来越困惑。
