...
**威胁模型（Threat Model）** 是系统假设谁会攻击、会怎么攻击、哪些资产最值得保护的风险图。直觉上它像作战前先画出的敌我地形图。工程上这意味着没有 threat model，安全评审很容易退化成零散列问题。

**严重度（Severity）** 是用来衡量某个问题真实危害级别的风险分级。直觉上它像医生先判断伤情轻重，再决定抢救优先级。工程上这意味着不是所有问题都该同等处理，团队资源要优先给真正高影响风险。

**修复方案（Remediation）** 是把已识别风险转化成具体修改动作的治理方案。直觉上它像不只指出哪里漏水，还要给出怎么补、先补哪块。工程上这意味着好的审计协作不止发现问题，还要把修复路径讲清楚。
