...
**Invariant** 是系统在所有合法执行路径下都应该保持成立的条件。直觉上它像机器运行时绝不能丢的底线。工程上这意味着只要不变量写清楚，很多安全与测试工作都会更有抓手。

**Assumption** 是分析或证明某条安全结论时默认成立的前提条件。直觉上它像证明题一开始写下的“已知条件”。工程上这意味着很多形式化或测试结论一旦离开假设范围，就不能被过度推广。

**形式化推理（Formal Reasoning）** 是用更严格的逻辑方法证明系统性质的分析方式。直觉上它像不再靠经验判断，而是尽量把结论变成可验证推导。工程上这意味着它适合验证关键不变量和边界，但前提是问题被表达得足够清楚。
