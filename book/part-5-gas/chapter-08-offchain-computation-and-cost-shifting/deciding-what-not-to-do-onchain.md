...
**链下计算（Offchain Computation）** 是把不必在链上完成的计算移到链下执行的设计选择。直觉上它像把重活放到场外做，只把必要结果带回链上。工程上这意味着你能显著降低成本，但必须重新面对结果可信性和验证边界问题。

**证明边界（Proof Boundary）** 是链上到底验证什么、链下到底负责什么的分界线。直觉上它像系统在说“我愿意相信到哪里，剩下必须给证据”。工程上这意味着成本转移成功与否，核心不在于搬走多少计算，而在于边界是否清楚。

**信任假设（Trust Assumption）** 是系统为了成立而默认某些参与方或数据来源可信的前提。直觉上它像你搭系统时默认哪些人不会乱来。工程上这意味着很多低成本方案其实是在用额外信任换便宜执行。
