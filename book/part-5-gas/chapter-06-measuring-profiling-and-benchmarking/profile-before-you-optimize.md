...
**基准测试（Benchmark）** 是用稳定方法反复测量关键路径成本的对比基线。直觉上它像给系统做标准化体测，而不是凭肉眼判断有没有变快。工程上这意味着优化必须以可重复比较的数据为依据。

**测量（Measurement）** 是用真实数据而不是直觉判断成本和性能的过程。直觉上它像别猜机器哪里慢，而是先拿仪表读数。工程上这意味着你应该先测后改，而不是先改再希望结果更好。

**回归（Regression）** 是后来改动让原本更好的成本或行为重新变差的现象。直觉上它像系统悄悄退步了，但如果没有对比基线你很难察觉。工程上这意味着性能和 gas 也需要像功能一样防回归。
