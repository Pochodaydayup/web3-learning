...
**Base Fee** 是区块基础费用，也是当前区块 gas 的底价要求。直觉上它像全网在这个区块统一收取的基础门票。工程上这意味着用户最终付多少钱，不只由你代码贵不贵决定，还和当时区块拥堵程度有关。

**Calldata 成本** 是把输入数据带上链所需要支付的成本部分。直觉上它像你不仅要为计算买单，还要为携带包裹的体积买单。工程上这意味着很多 L2 和批量交易设计，会特别在意输入数据到底有多大。

**L2 费用模型（L2 Fee Model）** 是二层网络把执行成本和数据成本组合成最终收费的方式。直觉上它像同一趟出行既要算车费，也要算过路和装载费用。工程上这意味着你在 L2 做 gas 判断时，不能直接套用 L1 的直觉。
