...
**CREATE** `CREATE` 是 EVM 用来创建新合约实例的核心操作码。直觉上它像正式把一份安装程序拿去生成一台新机器。工程上这意味着部署不仅是“发一笔交易”，而是经历 init code 执行后才留下真正可调用的运行时代码。

**Init Code** Init code 是合约部署时先执行、用于产出最终运行时代码的初始化代码。直觉上它像安装程序，先跑一遍再把真正的应用本体放到设备上。工程上这意味着 constructor 逻辑存在于部署阶段，而不会直接留在运行时代码里。

**Runtime Code** Runtime code 是部署完成后长期留在链上、真正响应调用的代码。直觉上它像应用安装完之后留在机器上的正式程序。工程上这意味着你调试日常调用时面对的是 runtime code，而不是部署时的 init code。
