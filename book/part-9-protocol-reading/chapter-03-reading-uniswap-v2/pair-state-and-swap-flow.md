...
**储备记账（Reserve Accounting）** 是协议如何记录池子里资产数量及其变化的会计逻辑。直觉上它像池子自己的总账本。工程上这意味着 AMM 和借贷协议很多安全与定价问题，本质上都是 reserve accounting 有没有被维护好。

**k 不变量（k Invariant）** 是 Uniswap V2 用乘积关系约束池子状态的核心不变量。直觉上它像池子内部那根决定价格与数量关系的拉力绳。工程上这意味着你读 swap 逻辑时，真正该盯住的不是表面转账，而是 k 是否仍然成立。

**手续费路径（Fee Path）** 是交易手续费在协议内部如何计入、累积和分配的流程。直觉上它像每笔交易留下的一点碎金最后流向哪里。工程上这意味着很多经济模型问题，必须沿着 fee path 才能看清。
