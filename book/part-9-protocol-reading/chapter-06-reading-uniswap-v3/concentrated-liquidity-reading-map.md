...
**Tick** 是 Uniswap V3 用来离散化价格区间的刻度单位。直觉上它像把连续价格尺切成一格一格的刻度。工程上这意味着你读 V3 时，很多流动性和费用逻辑都围绕 tick 展开，而不是直接围绕连续价格。

**流动性区间（Liquidity Range）** 是集中流动性在什么价格区间内参与做市的范围。直觉上它像 LP 不是把资金撒满整条价格线，而是只在自己选定的区间上岗。工程上这意味着收益和风险都会更集中，也让流动性管理更像主动仓位管理。

**Fee Growth** 是 V3 里手续费随时间和交易累积到头寸上的增长记录。直觉上它像头寸在运行过程中不断长出来的收益计数器。工程上这意味着读 V3 源码时，fee growth 是理解头寸价值变化的关键。
